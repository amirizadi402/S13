...
function write(response, badaneh, type, cookie, status) {
    if (!status) {
        status = 200;
    }
    if (cookie) {
            ...
            if (error) {
                ...
                fs.writeFile('database.txt', fileData, 'utf8', function (error) {
                    if (error) {
                        ...
                        write(response, "Signup done.", 'text');
                    }
                });
            }
        });
...
function signupControllerB(request, response, data) {
    console.log("inside insertToFile", data);
    data = JSON.parse(data);
        ...
        fs.readFile('database.txt', function (error, fileData) {
            if (error) {
                ...
                if (found === false) {
                    ...
                    write(response, "You already signed up.", 'text', null, 400);
                }
    ...
    if (data.user === undefined || data.pass === undefined) {
        ...
        fs.readFile('database.txt', function (error, fileData) {
            if (error) {
                ...
                if (found === false) {
                    write(response, "user not found", 'text', null, 400);
                }
                else{
                    let token = jwt.sign({ user: data.user }, secret);
...
function signupPageController(request, response) {
    console.log('page2controllerD 1')
    fs.readFile('./signupB.html', function (error, data) {
        console.log('page2controllerD 2');
        if (error) {
            write(response, 'FILE NOT FOUND.', 'text');
        }
...
function fileControllerC(request, response) {

    if (request.headers.cookie === undefined) {
        write(response, "Cookie is not provided", 'text');
    }
    else {
        let secret = "mysecretcode";
        let decodedCookie = jwt.verify(request.headers.cookie, secret);
...
let routes = {
    ...
    signupB: signupControllerB,
    loginA: loginControllerA,
    loginB: loginControllerB,
    verify: verifyCookie,
    signupPage: signupPageController,
    loginPage: loginPageController,
    fileC: fileControllerC
